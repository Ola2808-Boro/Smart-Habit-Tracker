...
def get_habit_tracker_detail_id(cursor,current_user_id:int):
...
def save_category(data:dict,current_user_id:int):
    ...
    try:
        if not data.get('category'):
            logging.warning(HTTP_LOG_MESSAGES[400].format(function_name="save_category", details="Missing ''category'"))
        ...
        if data['category'] in [item[0] for item in results]:
            logging.warning(HTTP_LOG_MESSAGES[409].format(function_name="save_category", details="Category already exists."))
...
def save_habit(data:dict,current_user_id:int):
    ...
    try:
        if not data.get('habit'):
            logging.warning(HTTP_LOG_MESSAGES[400].format(function_name="save_habit", details="Missing 'habit'"))
        ...
        if data['habit'] in [item[0] for item in results]:
            logging.warning(HTTP_LOG_MESSAGES[409].format(function_name="save_habit", details="Habit already exists."))
            ...
            for category in data['category']:
                if category==[]:
                    ...
                    if category_id is None:
                        logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="save_habit", details="Category_id not found."))
...
def get_habit(current_user_id:int):
    ...
    try:
        ...
        for result in habit_results:
            ...
            category_id_results=get_category_id_by_habit_id(cursor,current_user_id,habit_id)
            #handle this exception
            if category_id_results is None:
                logging.warning(HTTP_LOG_MESSAGES[204].format(function_name="get_habit", details="Category_id_results not found."))
                #return 204, "Category_id_results not found.", []
            categories=[]
            for category_id in category_id_results:
                category_name=get_category_name(cursor,current_user_id,category_id)
                if category_name is None:
                    logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="get_habit", details="Category_name not found."))
...
def get_task(data:dict,current_user_id:int):
    ...
    try:
        if not data.get('selectedDate') :
            logging.warning(HTTP_LOG_MESSAGES[400].format(function_name="get_task", details="Missing 'selectedDate'"))
            return 400, "Missing 'selectedDate' in request."
        cursor=conn.cursor()
        habit_tracker_detail_id=get_habit_tracker_detail_id(cursor,current_user_id)
        if habit_tracker_detail_id is None:
            logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="get_task", details="Habit_tracker_detail_id not found."))
            return 404, "Habit_tracker_detail_id not found.", None
        sql_select_habit_id="""
        ...
        for habit_id,duration in  habit_id_duration_results:
            ...
            if habit_name is None:
                    logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="get_task", details="Habit_name not found."))
                    return 404, "Habit_name not found.", None
            category_id_results=get_category_id_by_habit_id(cursor,current_user_id,habit_id)
            if category_id_results is None:
                logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="get_task", details="Category_id_results not found."))
                return 404, "Category_id_results not found.", None
            categories=[]
            for category_id in category_id_results:
                category_name=get_category_name(cursor,current_user_id,category_id)
                if category_name is None:
                    logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="get_task", details="Category_name not found."))
                    return 404, "Category_name not found.", None
                categories.append(category_name)
...
def save_task(data:dict,current_user_id:int):
    ...
    try:
        if not data.get('task') or not data.get('time'):
            logging.warning(HTTP_LOG_MESSAGES[400].format(function_name="save_task", details="Missing 'task' or 'time'"))
        ...
        if habit_id is None:
            logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="save_task", details="Habit not found."))
            return 404, "Habit not found."
        habit_tracker_detail_id=get_habit_tracker_detail_id(cursor,current_user_id)
        if habit_tracker_detail_id is None:
            logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="save_task", details="Habit_tracker_detail_id not found."))
        ...
        if data['time']=='0:00':
            ...
            logging.info(HTTP_LOG_MESSAGES[201].format(function_name='save_task-insert'))
        else:
            sql_update="""
            ...
            logging.info(HTTP_LOG_MESSAGES[201].format(function_name='save_task-update'))
        conn.commit()
...
def remove_task(data:dict,current_user_id:int):
    conn=create_connection()
    try:
        if not data.get('task') or not data.get('category') or not data.get('selectedDate'):
            logging.warning(HTTP_LOG_MESSAGES[400].format(function_name="remove_task", details="Missing 'task' or 'category' or 'selectedDate'"))
        ...
        habit_id=get_habit_id(cursor=cursor,user_id=current_user_id,habit_name=data['task'])
        if habit_id is None:
            logging.warning(HTTP_LOG_MESSAGES[404].format(function_name="remove_task", details="Habit not found."))
            return 404, "Habit not found."
        sql_delete_habit="""
