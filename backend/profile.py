...
def update_avatar_image(data: dict, current_user_id: int):
    ...
    try:
        ...
        cursor.execute(sql_insert_mood, (Binary(image_bytes), current_user_id))
        conn.commit()
...
def get_avatar_image(current_user_id: int):
    ...
    try:
        ...
        """
        cursor.execute(sql_select_mood, (current_user_id,))
        results = cursor.fetchone()[0]
        if results is not None:
            print(f"Avatar results: {results}")
        ...
        return 201, "Succesfully retrievedd navatar image", results
    except ProgrammingError as e:
        logging.error(f"SQL syntax or logic error: {e}")
